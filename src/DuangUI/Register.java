	...
	private void createContents() {
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				setphoto set=new setphoto(getParent(), getStyle());
				set.open();
				image_url=set.image_urlString;
				System.out.println(image_url);
			}
		});
		button.setText("选择头像");
		...
		if(jg.checkUsername(username)&&jg.checkPassword(password)&&jg.checkPassword_1(password,password_1)
			...
			if(!(jg.checkUsername(username))){
				errorString1 = "用户名，重复或者不超过6位";
			}
			else{
				errorString1 = "";
			}
			if(!(jg.checkPassword(password))){
				errorString2 = "密码至少为6位";
			}
			else{
				errorString2 = "";
			}
			...
			JOptionPane.showMessageDialog(null, errorString+"请重新输入！","错误", JOptionPane.ERROR_MESSAGE);
		}	   
				}
