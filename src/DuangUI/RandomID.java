...
public class RandomID {
	...
	private String generation(int tableno) {
		...
		long timeOfDate = time.get(Calendar.YEAR) * 10000
				+ (time.get(Calendar.MONTH) + 1) * 100
				+ time.get(Calendar.DAY_OF_MONTH);
		long timeofDay = time.get(Calendar.HOUR_OF_DAY) * 10000
				+ time.get(Calendar.MINUTE) * 100 + time.get(Calendar.SECOND);
		long millsecond = time.get(Calendar.MILLISECOND); // 生成时间年月

		// 时间字符串
		String timeString = (String.valueOf(timeOfDate)).concat((String
				.valueOf(timeofDay)).concat(String.valueOf(millsecond)));
